        ...
        if(!usern.getText().toString().isEmpty() && !passw.getText().toString().isEmpty() && !confirmpass.getText().toString().isEmpty()) {
            if (passw.getText().toString().equals(confirmpass.getText().toString())) {
                ...
                if (c1.getCount() > 0 || c2.getCount() > 0) {
                    ...
                    db.execSQL("INSERT INTO profesor(username,password) VALUES('" + usern.getText().toString() + "','" + passw.getText().toString() + "' );");
                    Toast.makeText(this,"Register like professor.",Toast.LENGTH_LONG).show();
                    c1.close();
                    c2.close();
                    Intent intent = null;
